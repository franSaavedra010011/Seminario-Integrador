...
let PermisosGuard = class PermisosGuard {
    ...
    canActivate(context) {
        const requiredPermisos = this.reflector.getAllAndOverride(permisos_decorator_1.PERMISOS_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        if (usuario.roles?.some((r) => r.toUpperCase() === 'ADMIN') ||
            usuario.rol?.toUpperCase() === 'ADMIN') {
            return true;
        }
        if (usuario.permisos && Array.isArray(usuario.permisos)) {
            const permisosUsuario = usuario.permisos.map((p) => p.toLowerCase());
